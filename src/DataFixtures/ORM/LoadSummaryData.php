...
class LoadSummaryData implements FixtureInterface
{
    public function load(ObjectManager $manager)
    {
        ...
        $summary1 = $summaryFactory->createFromArray([
            ...
            'slug' => 'summary-michelle-dufour',
            'availability' => JobDuration::PART_TIME,
            'contactEmail' => 'michelle.dufour.2@example.ch',
            'contributionWish' => Contribution::VOLUNTEER,
            'jobLocation' => JobLocation::ON_REMOTE,
        ...
        // Experiances
        $experiance11 = new JobExperience();
        $experiance11->setCompany('Institut KNURE');
        $experiance11->setDescription('Bio-informaticien dans l\'institut KNURE');
        $experiance11->setOnGoing(true);
        $experiance11->setStartedAt(new \DateTime('2007-04-28 00:00:00'));
        $experiance11->setDisplayOrder(1);
        $experiance11->setCompanyFacebookPage('https://www.facebook.com/khure-fake');
        $experiance11->setCompanyTwitterNickname('khureBioInformatique');
        $experiance11->setContract(Contract::PERMANENT);
        $experiance11->setDuration(JobDuration::FULL_TIME);
        $experiance11->setLocation('Genève');
        $experiance11->setPosition('Bio-informaticien');
        $experiance11->setWebsite('http://khure.bioinformatique.ch');
        $experiance11->setSummary($summary1);
        $manager->persist($experiance11);

        $experiance12 = new JobExperience();
        $experiance12->setCompany('Univérsité Lyon 1');
        $experiance12->setDescription('Professeur à l\'université');
        $experiance12->setOnGoing(false);
        $experiance12->setStartedAt(new \DateTime('1995-10-26 00:00:00'));
        $experiance12->setEndedAt(new \DateTime('2006-01-26 00:00:00'));
        $experiance12->setDisplayOrder(2);
        $experiance12->setCompanyFacebookPage('https://www.facebook.com/lyon1-fake');
        $experiance12->setCompanyTwitterNickname('lyon1-fake');
        $experiance12->setContract(Contract::PERMANENT);
        $experiance12->setDuration(JobDuration::FULL_TIME);
        $experiance12->setLocation('Lyon');
        $experiance12->setPosition('Professeur');
        $experiance12->setWebsite('http://lyon.bioinformatique.fr');
        $experiance12->setSummary($summary1);
        $manager->persist($experiance12);
        ...
        $summary2 = $summaryFactory->createFromArray([
            ...
            'slug' => 'summary-carl-mirabeau',
            'availability' => JobDuration::PUNCTUALLY,
        ...
        $summary3 = $summaryFactory->createFromArray([
            ...
            'slug' => 'summary-jacques-picar',
            'availability' => JobDuration::FULL_TIME,
